...
model_opt = json.loads(open(argv[1]))
layers = model_opt["layers"]

# 3 layer fully connected network
if not "layers" in model_opt.keys():
    print("no layers in json file {}.".format(argv[1]))
    exit(1)
...
class NNUE(pl.LightningModule):
  ...
  def __init__(self, feature_set, lambda_=1.0):
    ...
    self.input = nn.Linear(feature_set.num_features, layers[1])

    self.layers = []
    ctr = 0
    for i in range(len(layers[:-1])):
        exec("self.l{} = nn.Linear({}, {})".format(ctr, layers[ctr], layers[ctr+1]))
        self.layers.append(exec("self.l{}".format(ctr)))
